...
modelsummary(hurdle(actual_abu ~ mean_sst,
       data = pres_abs,
       dist = "negbin",
       zero.dist = "binomial"))
```
...
After running a hurdle model on my simulated data, I can apply it to my actual data. I will add the upwelling predictor variable back in to test my hypothesis. From there I can make a prediction grid and bootstrap to get the confidence intervals.
```{r}
...
boot %>% 
  group_by(boot) %>% 
  summarise(upper_ci = quantile(count, 0.025),
            lower_ci = quantile(count, 0.975)) %>%
  dplyr::select(-boot) %>% 
  kableExtra::kable(col.names = c("Upper CI", "Lower CI"))
```
```{r}
...
```{r}
lower_ci = quantile(boot$count, 0.025)
upper_ci = quantile(boot$count, 0.975)
``` 
